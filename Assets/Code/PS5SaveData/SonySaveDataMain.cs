...
public class SonySaveDataMain : MonoBehaviour, IScreen
{
    ...
    void Initialize()
    {
        ...
        // We do InitializeSaveData() at the start of the game not here
    ...
    {
        print("API Called = (" + callbackEvent.ApiCalled + ") : Request Id = (" + callbackEvent.RequestId + ") : Calling User Id = (0x" + callbackEvent.UserId.ToString("X8") + ")");
    ...
    void Update()
    {
        ...
        //if (!User.IsActiveUserAdded) return;
        ...
        if (m_FileOps.ResponseFormed)
        {
            OnScreenLog.Add("ResponseFormed ");

            if (SL != null)
            {

              
                SL.SaveLoadCurrent.MENU_LOAD_DATA(m_FileOps.Response.menuData);

                Debug.Log(m_FileOps.Response.largeData);

                SL.SaveLoadCurrent.MAIN_LOAD_DATA(m_FileOps.Response.largeData);
                SL.SaveExists = true;
                _menu.LoadMenu();
                ...
                if (_DayAndNight != null)
                {
                    ...
                    _DayAndNight.DayTime = SL.SaveLoadCurrent.DayTime;
                }

                
            }
           m_FileOps.ResponseFormed = false;
        }
    ...
    void MenuMain()
    {
        m_MenuMain.Update();
            ...
            if (m_MenuMain.AddItem("Terminate Library", true))
            {
                TerminateLibrary();
            }

            if (m_MenuMain.AddItem("Dialogs", true))
            {
                m_MenuStack.PushMenu(m_Dialogs.GetMenu());
            }

            if (m_MenuMain.AddItem("Search", true))
            {
                m_MenuStack.PushMenu(m_Search.GetMenu());
            }

            if (m_MenuMain.AddItem("Start Auto-save", true))
            {
                StartAutoSave();
            }

            if (m_MenuMain.AddItem("Load Auto-save", true))
            {
                StartAutoSaveLoad();
            }
    ...
    public void StartAutoSave()
    {
        ...
        OnScreenLog.Add("StartAutoSave1");
        ...
        UInt64 newSaveDataBlocks2 = SonySaveDataMain.TestBlockSize;
        ...
        fileRequest.LargeDataStart = _menu.CurrentSlotNumber * PS5SaveLoad.saveDataSize;
        OnScreenLog.Add("Set LargeDataStart  " + fileRequest.LargeDataStart);
        ...
        fileRequest.menuData = MenuSavedata;
        fileRequest.largeData = Largedata;

        fileRequest.SaveAll = SaveAll;
        ...
        SonySaveDataMain.StartSaveDataCoroutine(SaveData.AutoSaveProcess.StartAutoSaveProcess(userId, newItem, newDirName, newSaveDataBlocks, saveDataParams, fileRequest, fileResponse, backup, HandleAutoSaveError, newSaveDataBlocks2));
    }
    ...
    {
        OnScreenLog.Add("StartAutoSaveLoad");
        // Get the user id for the saves
        int userId = User.GetActiveUserId;
        ...
        SonySaveDataMain.StartSaveDataCoroutine(SaveData.AutoSaveProcess.StartAutoSaveLoadProcess(userId, dirName, fileRequest, fileResponse, HandleAutoSaveError, newSaveDataBlocks2));
    }
