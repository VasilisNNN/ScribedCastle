...
public class SonySaveDataMain : MonoBehaviour, IScreen
{
    ...
    void Update()
    {
        ...
        if (m_FileOps.ResponseFormed)
        {
            ...
            if (SL != null)
            {
                ...
                SL.PS5_MENU_LoadLists(m_FileOps.Response.menuData);

                print("m_FileOps.Response.largeData.Length " + m_FileOps.Response.largeData.Length);

                // ExampleReadFilesResponse.LargeDataStart = SaveLoad.saveDataSize * _menu.CurrentSlotNumber;
                SL.PS5_Load(m_FileOps.Response.largeData);
                SL.SaveExists = true;
                _menu.LoadMenu();

                if (_DayAndNight != null)
                {
                    print("pl.DayNight.Day " + SL.DayTime);
                    _DayAndNight.Day = SL.DayTime;
                }
    ...
    public void StartAutoSave()
    {
        ...
        fileRequest.LargeDataStart = _menu.CurrentSlotNumber * SaveLoad.saveDataSize;
