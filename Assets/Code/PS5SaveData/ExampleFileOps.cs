...
public class ExampleWriteFilesRequest : FileOps.FileOperationRequest
{
    ...
    public byte[] largeData = new byte[SaveLoad.saveDataSize];
    ...
    public override void DoFileOperations(Mounting.MountPoint mp, FileOps.FileOperationResponse response)
    {
      ...
      fileResponse.totalFileSizeWritten += info.Length;
        ...
        using (FileStream fs = new FileStream(outpath4, FileMode.OpenOrCreate, FileAccess.Write, FileShare.None, SaveLoad.saveDataSize*7)) // ; File.Create(outpath4, 1024 * 1024 * 5)) // File.OpenWrite(outpath3))
        {
...
public class ExampleReadFilesRequest : FileOps.FileOperationRequest
{
    public override void DoFileOperations(Mounting.MountPoint mp, FileOps.FileOperationResponse response)
    {
        ...
        fileResponse.largeData = new byte[SaveLoad.saveDataSize*7];
        int totalRead = 0;

        // Example of updating the progress value.
        using (FileStream fs = new FileStream(outpath4, FileMode.Open, FileAccess.Read, FileShare.None, SaveLoad.saveDataSize*7)) // File.OpenRead(outpath3))
        {
